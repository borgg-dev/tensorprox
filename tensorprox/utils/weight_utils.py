...
def process_weights_for_netuid(
    ...
    if non_zero_weights.size == 0:
        bittensor.logging.warning("No non-zero weights — no miners have earned weight.")
        if burn_uid is not None:
            # No miners earned weight — burn everything (100% to burn UID)
            bittensor.logging.info(f"All miner weights zero — setting burn to 1.0 (normally {burn_weight})")
            return np.array([burn_uid]), np.array([1.0])
        else:
            # Nothing to set — return empty arrays to signal "skip"
            return np.array([], dtype=int), np.array([], dtype=np.float32)

    elif metagraph.n < min_allowed_weights:
        bittensor.logging.warning("metagraph.n < min_allowed_weights, returning all ones.")
        if burn_uid is not None:
