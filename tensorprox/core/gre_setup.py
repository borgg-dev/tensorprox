...
import os
from tensorprox.utils.utils import run_cmd_async
import time
import re
...
class GRESetup(BaseModel):
    ...
    def detect_primary_interface(self):
        ...
        for interface in common_interfaces:
            ...
            result = run_cmd_async(["ip", "link", "show", interface], quiet=True)
            if result.returncode == 0:
                # Check if it has an IP
                ip_result = run_cmd_async(["ip", "-o", "-4", "addr", "show", "dev", interface], quiet=True)
        ...
        route_result = run_cmd_async(["ip", "-o", "route", "get", "1.1.1.1"], quiet=True)
        ...
        all_interfaces_result = run_cmd_async(["ip", "link", "show"], quiet=True)
    ...
    def flush_device(self, dev):
        ...
        run_cmd_async(["ip", "link", "set", dev, "down"], quiet=True)
        run_cmd_async(["ip", "link", "del", dev], quiet=True)

    def clean_policy_routing(self):
        """Clean existing policy routing rules and tables"""
        # Save original rules that aren't ours
        existing_rules = run_cmd_async(["ip", "rule", "list"], quiet=True).stdout
        
        # First, delete any custom rules we've set (lookups to table 100-110)
        for i in range(100, 111):
            run_cmd_async(["ip", "rule", "del", "lookup", str(i)], quiet=True)
        
        # Flush custom routing tables
        for i in range(100, 111):
            run_cmd_async(["ip", "route", "flush", "table", str(i)], quiet=True)
        
        # Restore system default rules (just to be safe)
        run_cmd_async(["ip", "rule", "add", "from", "all", "lookup", "local", "pref", "0"], quiet=True)
        run_cmd_async(["ip", "rule", "add", "from", "all", "lookup", "main", "pref", "32766"], quiet=True)
        run_cmd_async(["ip", "rule", "add", "from", "all", "lookup", "default", "pref", "32767"], quiet=True)
    ...
    def detect_system_capabilities(self):
        ...
        virt_check = run_cmd_async(["systemd-detect-virt"], quiet=True)
        ...
        # Get total memory
        mem_info = run_cmd_async(["grep", "MemTotal", "/proc/meminfo"], quiet=True)
        ...
        # Check NUMA topology
        numa_check = run_cmd_async(["lscpu"], quiet=True)
        ...
        if primary_interface:
            driver_info = run_cmd_async(["ethtool", "-i", primary_interface], quiet=True)
            ...
            speed_info = run_cmd_async(["ethtool", primary_interface], quiet=True)
        ...
        # Check XDP support
        xdp_check = run_cmd_async(["grep", "CONFIG_XDP_SOCKETS=y", "/boot/config-$(uname -r)"], quiet=True)
        if xdp_check.returncode == 0:
            ...
            if not capabilities["is_virtualized"]:
                native_check = run_cmd_async(["ip", "link", "set", "dev", primary_interface, "xdp", "off"], quiet=True)
        ...
        dpdk_check = run_cmd_async(["apt-cache", "search", "^dpdk$"], quiet=True)
    ...
    def calculate_resource_allocation(self, capabilities, node_type):
        ...
        log("[INFO] Resource allocation for {0}: {1} DPDK cores, {2} reserved cores, {3}GB hugepages".format(
            node_type, resource_plan["dpdk_cores"], resource_plan["reserve_cores"], resource_plan["hugepages_gb"]), level=1)
    ...
    def optimize_kernel_params(self):
        ...
        run_cmd_async(["modprobe", "ip_gre"], quiet=True)
        run_cmd_async(["modprobe", "ipip"], quiet=True)
        run_cmd_async(["modprobe", "xdp"], quiet=True)
        run_cmd_async(["modprobe", "veth"], quiet=True)
        
        # Critical performance parameters for high throughput
        run_cmd_async(["sysctl", "-w", "net.core.rmem_max=268435456"])  # 256 MB
        run_cmd_async(["sysctl", "-w", "net.core.wmem_max=268435456"])  # 256 MB
        run_cmd_async(["sysctl", "-w", "net.core.optmem_max=134217728"])  # 128 MB
        run_cmd_async(["sysctl", "-w", "net.ipv4.tcp_rmem=4096 87380 134217728"])
        run_cmd_async(["sysctl", "-w", "net.ipv4.tcp_wmem=4096 65536 134217728"])
        run_cmd_async(["sysctl", "-w", "net.core.netdev_max_backlog=1000000"])
        run_cmd_async(["sysctl", "-w", "net.core.somaxconn=1048576"])
        
        # Enable IP forwarding
        run_cmd_async(["sysctl", "-w", "net.ipv4.ip_forward=1"])
        
        # Disable ICMP redirects completely (prevent routing loops)
        run_cmd_async(["sysctl", "-w", "net.ipv4.conf.all.accept_redirects=0"])
        run_cmd_async(["sysctl", "-w", "net.ipv4.conf.default.accept_redirects=0"])
        run_cmd_async(["sysctl", "-w", "net.ipv4.conf.all.send_redirects=0"])
        run_cmd_async(["sysctl", "-w", "net.ipv4.conf.default.send_redirects=0"])
        
        # Tunnel specific parameters
        run_cmd_async(["sysctl", "-w", "net.ipv4.conf.all.accept_local=1"])
        run_cmd_async(["sysctl", "-w", "net.ipv4.conf.default.accept_local=1"])
        run_cmd_async(["sysctl", "-w", "net.ipv4.ip_forward_use_pmtu=1"])
        
        # Optimize for XDP performance
        run_cmd_async(["sysctl", "-w", "net.core.bpf_jit_enable=1"])
        run_cmd_async(["sysctl", "-w", "net.core.bpf_jit_harden=0"])
        run_cmd_async(["sysctl", "-w", "net.core.bpf_jit_kallsyms=1"])
        
        # Optimize network device budget for throughput
        run_cmd_async(["sysctl", "-w", "net.core.netdev_budget=50000"])
        run_cmd_async(["sysctl", "-w", "net.core.netdev_budget_usecs=5000"])
        
        # Optimize flow director for direct hardware mapping
        run_cmd_async(["sysctl", "-w", "net.core.flow_limit_table_len=8192"])
    ...
    def optimize_kernel_for_overlay_network(self):
        ...
        run_cmd_async(["sysctl", "-w", "net.ipv4.tcp_congestion_control=bbr"], quiet=True)
        
        # Increase PPS handling capacity
        run_cmd_async(["sysctl", "-w", "net.core.netdev_budget=1000"], quiet=True)
        run_cmd_async(["sysctl", "-w", "net.core.netdev_budget_usecs=2000"], quiet=True)
        ...
        run_cmd_async(["sysctl", "-w", "net.core.rmem_default=16777216"], quiet=True)  # 16MB default
        run_cmd_async(["sysctl", "-w", "net.core.wmem_default=16777216"], quiet=True)  # 16MB default
        
        # Increase connection tracking table size for tunneled traffic
        run_cmd_async(["sysctl", "-w", "net.netfilter.nf_conntrack_max=2097152"], quiet=True)
        
        # Enable direct packet access in the fast path
        run_cmd_async(["sysctl", "-w", "net.core.bpf_jit_enable=2"], quiet=True)
        
        # Optimize TCP for tunnels
        run_cmd_async(["sysctl", "-w", "net.ipv4.tcp_timestamps=1"], quiet=True)
        run_cmd_async(["sysctl", "-w", "net.ipv4.tcp_sack=1"], quiet=True)
        
        # Disable swap for networking performance
        run_cmd_async(["sysctl", "-w", "vm.swappiness=0"], quiet=True)
        
        # Optimize memory allocation for network buffers
        run_cmd_async(["sysctl", "-w", "vm.min_free_kbytes=65536"], quiet=True)
        run_cmd_async(["sysctl", "-w", "vm.zone_reclaim_mode=0"], quiet=True)
    ...
    def optimize_cpu_irq_for_tunnel(self, resource_plan):
        ...
        if resource_plan["isolated_cpus"]:
            ...
            try:
                ...
                run_cmd_async(["mv", "/etc/default/grub.new", "/etc/default/grub"], quiet=True)
        ...
        for irq in irqs:
            run_cmd_async(["chrt", "-f", "-p", "99", irq], quiet=True)
        
        # Enable IRQ balancing for network queues
        run_cmd_async(["systemctl", "stop", "irqbalance"], quiet=True)
    ...
    def optimize_virtio_for_tunneling(self):
        ...
        cpu_count = multiprocessing.cpu_count()
        run_cmd_async(["ethtool", "-L", primary_interface, "combined", str(max(1, cpu_count - 1))], quiet=True)
        
        # Increase descriptor ring size for virtio
        run_cmd_async(["ethtool", "-G", primary_interface, "rx", "1024", "tx", "1024"], quiet=True)
        
        # Optimize virtio queue processing
        run_cmd_async(["ethtool", "-C", primary_interface, "adaptive-rx", "on", "adaptive-tx", "on"], quiet=True)
        
        # Enable offloads that virtio supports
        run_cmd_async(["ethtool", "--offload", primary_interface, "rx-checksumming", "on"], quiet=True)
        run_cmd_async(["ethtool", "--offload", primary_interface, "tx-checksumming", "on"], quiet=True)
        run_cmd_async(["ethtool", "--offload", primary_interface, "sg", "on"], quiet=True)
        run_cmd_async(["ethtool", "--offload", primary_interface, "tso", "on"], quiet=True)
        run_cmd_async(["ethtool", "--offload", primary_interface, "gso", "on"], quiet=True)
        run_cmd_async(["ethtool", "--offload", primary_interface, "gro", "on"], quiet=True)
        
        # Enable Busy Polling for virtio - reduces latency at cost of CPU
        run_cmd_async(["sysctl", "-w", "net.core.busy_read=50"], quiet=True)
        run_cmd_async(["sysctl", "-w", "net.core.busy_poll=50"], quiet=True)
        
        # Optimize I/O scheduling for virtio
        run_cmd_async(["echo", "none", ">", f"/sys/block/vda/queue/scheduler"], shell=True, quiet=True)
    ...
    def optimize_tunnel_interface(self, interface):
        ...
        run_cmd_async(["sysctl", "-w", f"net.ipv4.conf.{interface}.rp_filter=0"])
        
        # Enable source routing for the tunnel interface
        run_cmd_async(["sysctl", "-w", f"net.ipv4.conf.{interface}.accept_source_route=1"])
        
        # Allow local routing on the tunnel interface
        run_cmd_async(["sysctl", "-w", f"net.ipv4.conf.{interface}.route_localnet=1"])
        
        # Disable ICMP redirects on the tunnel interface
        run_cmd_async(["sysctl", "-w", f"net.ipv4.conf.{interface}.accept_redirects=0"])
        run_cmd_async(["sysctl", "-w", f"net.ipv4.conf.{interface}.send_redirects=0"])
        
        # Increase the interface queue length for high throughput
        run_cmd_async(["ip", "link", "set", "dev", interface, "txqueuelen", "100000"], quiet=True)
        
        # Additional tunnel-specific optimizations
        run_cmd_async(["sysctl", "-w", f"net.ipv4.conf.{interface}.accept_local=1"])
        run_cmd_async(["sysctl", "-w", f"net.ipv4.conf.{interface}.forwarding=1"])
        
        # Set MTU discovery to "want"
        run_cmd_async(["sysctl", "-w", f"net.ipv4.conf.{interface}.mtu_probing=1"])
        
        # Explicitly configure GRO/GSO for tunnel
        run_cmd_async(["ethtool", "-K", interface, "gro", "on"], quiet=True)
        run_cmd_async(["ethtool", "-K", interface, "gso", "on"], quiet=True)
        
        # Set TSO on when possible for tunnels
        run_cmd_async(["ethtool", "-K", interface, "tso", "on"], quiet=True)
    ...
    def update_apt_repositories(self, max_retries=3, switch_mirrors=True):
        ...
        for attempt in range(max_retries):
            ...
            update_result = run_cmd_async(["apt-get", "update", "-y"], quiet=True, timeout=120)
    ...
    def try_switch_mirrors(self):
        ...
        try:
            ...
            if not os.path.exists("/etc/apt/sources.list.backup"):
                run_cmd_async(["cp", "/etc/apt/sources.list", "/etc/apt/sources.list.backup"], quiet=True)
    ...
    def install_packages_resilient(self, package_list, max_retries=3, timeout=600):
        ...
        for attempt in range(max_retries):
            ...
            install_result = run_cmd_async(install_cmd, shell=True, quiet=True, timeout=timeout)
            ...
            if install_result.returncode == 0:
                ...
                run_cmd_async(["dpkg", "--configure", "-a"], quiet=True)
    ...
    def install_afxdp_dependencies(self):
        ...
        dpkg_check = run_cmd_async(["pgrep", "dpkg"], quiet=True)
        apt_check = run_cmd_async(["pgrep", "apt"], quiet=True)
        ...
        if dpkg_check.returncode == 0 or apt_check.returncode == 0:
            ...
            run_cmd_async(["dpkg", "--configure", "-a"], quiet=True, timeout=120)
        ...
        run_cmd_async(["pip3", "install", "pyroute2"], quiet=True)
        
        # Load necessary kernel modules
        run_cmd_async(["modprobe", "xdp"], quiet=True)
        run_cmd_async(["modprobe", "veth"], quiet=True)
        run_cmd_async(["modprobe", "tun"], quiet=True)
        
        # Enable BPF JIT compilation
        run_cmd_async(["sysctl", "-w", "net.core.bpf_jit_enable=1"], quiet=True)
    ...
    def setup_hugepages(self, resource_plan):
        ...
        if os.path.exists("/sys/kernel/mm/hugepages/hugepages-1048576kB"):
            ...
            run_cmd_async(["sysctl", "-w", f"vm.nr_hugepages={num_pages}"])
        ...
        if not os.path.exists("/mnt/huge"):
            run_cmd_async(["mkdir", "-p", "/mnt/huge"])
        
        # Mount hugepages
        run_cmd_async(["mount", "-t", "hugetlbfs", "nodev", "/mnt/huge"])
        ...
        run_cmd_async(["mkdir", "-p", "/dev/hugepages/dpdk"], quiet=True)
        
        # Verify hugepages setup
        hugepages_check = run_cmd_async(["grep", "Huge", "/proc/meminfo"], show_output=True)
    ...
    """
        ...
        if resource_plan["isolated_cpus"]:
            run_cmd_async(["systemctl", "set-property", "dpdk.service", f"CPUAffinity={resource_plan['isolated_cpus']}"], quiet=True)
        ...
        run_cmd_async(["sysctl", "-w", "kernel.sched_rt_runtime_us=-1"], quiet=True)
        
        # Configure virtio for optimal DPDK performance
        primary_interface, _ = self.detect_primary_interface()
        run_cmd_async(["ethtool", "--offload", primary_interface, "rx", "on", "tx", "on"], quiet=True)
        run_cmd_async(["ethtool", "--offload", primary_interface, "sg", "on", "tso", "on", "gso", "on", "gro", "on"], quiet=True)
    ...
    def install_dpdk_dependencies(self):
        ...
        dpdk_check = run_cmd_async(["dpdk-devbind.py", "--status"], quiet=True)
        ...
        if not dpdk_installed:
            ...
            run_cmd_async(["nohup bash -c 'DEBIAN_FRONTEND=noninteractive apt-get install -y --fix-missing dpdk dpdk-dev dpdk-tools dpdk-runtime > /tmp/dpdk_install.log 2>&1 &'"],
    ...
    def create_optimized_xdp_program(self, interface, node_type):
        """Create optimized XDP program for virtio environments"""
        log("[INFO] Creating optimized XDP program for {0}".format(node_type), level=1)
    ...
    """
        ...
        program_file = os.path.join(self.xdp_program_dir, "{0}_xdp.c".format(node_type))
        with open(program_file, "w") as f:
            f.write(xdp_program)
        
        # Install clang and LLVM if needed
        run_cmd_async(["apt-get", "install", "-y", "clang", "llvm"], quiet=True)
        
        # Compile the XDP program
        object_file = os.path.join(self.xdp_program_dir, "{0}_xdp.o".format(node_type))
        compile_result = run_cmd_async(["clang", "-O2", "-g", "-Wall", "-target", "bpf", "-c", program_file, "-o", object_file], quiet=True)
        ...
        if compile_result.returncode == 0:
            ...
            if "virtio" in driver_info.stdout:
                ...
                load_result = run_cmd_async(["ip", "link", "set", "dev", primary_interface, "xdpgeneric", "obj", object_file, "sec", "xdp"], show_output=True, quiet=False)
            else:
                # Try native mode first, fall back to generic
                load_result = run_cmd_async(["ip", "link", "set", "dev", primary_interface, "xdp", "obj", object_file, "sec", "xdp"], quiet=True)
                if load_result.returncode != 0:
                    log("[INFO] Native XDP failed, falling back to generic XDP mode", level=1)
                    load_result = run_cmd_async(["ip", "link", "set", "dev", primary_interface, "xdpgeneric", "obj", object_file, "sec", "xdp"], quiet=True)
    ...
    def create_enhanced_afxdp_program(self, interface, node_type, resource_plan):
        """Create AF_XDP program optimized for VM environments"""
        log("[INFO] Creating enhanced AF_XDP program for {0}".format(node_type), level=1)
        
        # Create the Python AF_XDP program
        program_file = os.path.join(self.xdp_program_dir, "{0}_afxdp.py".format(node_type))
    ...
    LOG_FILE = "{self.xdp_log_dir}/{node_type}_afxdp.log"
    USE_ZEROCOPY = True
    ...
    """
        ...
        service_file = "/etc/systemd/system/afxdp-{0}.service".format(node_type)
    ...
    """.format(node_type, self.xdp_program_dir)
        
        with open(service_file, "w") as f:
            f.write(service_content)
        
        # Reload systemd and enable/start the service
        run_cmd_async(["systemctl", "daemon-reload"], quiet=True)
        run_cmd_async(["systemctl", "enable", "afxdp-{0}".format(node_type)], quiet=True)
        run_cmd_async(["systemctl", "start", "afxdp-{0}".format(node_type)], quiet=True)
        
        log("[INFO] Enhanced AF_XDP acceleration enabled for {0} on {1}".format(node_type, interface), level=1)
        return True

    def setup_enhanced_acceleration(self, node_type, interface, resource_plan):
        """Set up enhanced hybrid acceleration with intelligent scaling and improved reliability"""
        log("[INFO] Setting up enhanced acceleration for {0}".format(node_type), level=1)
        ...
        self.create_optimized_xdp_program(interface, node_type)
        ...
        self.create_enhanced_afxdp_program(interface, node_type, resource_plan)
        
        log("[INFO] Enhanced acceleration setup complete for {0}".format(node_type), level=1)
        return True
