...
class GRESetup(BaseModel):
    ...
    def configure_node(self, machine_name, moat_ip):
        ...
        resource_plan = self.calculate_resource_allocation(capabilities, machine_name)
        ...
        self.flush_device(f"ipip-{machine_name}")
        ...
        self.run_cmd(["ip", "tunnel", "add", f"ipip-{machine_name}", "mode", "ipip", 
                "local", gre_ip, "remote", ipip_ip, "ttl", "inherit"], check=True)
        
        self.run_cmd(["ip", "link", "set", f"ipip-{machine_name}", "mtu", str(IPIP_MTU)])
        self.run_cmd(["ip", "addr", "add", f"{overlay_ip}/32", "dev", f"ipip-{machine_name}"])
        self.run_cmd(["ip", "link", "set", f"ipip-{machine_name}", "up"])
        
        self.optimize_tunnel_interface(f"ipip-{machine_name}")
        ...
        self.run_cmd(["ip", "rule", "add", "iif", f"ipip-{machine_name}", "lookup", "100", "pref", "100"])
        self.run_cmd(["ip", "route", "add", "default", "via", ipip_ip, "dev", "gre-moat", "table", "100"])
        self.run_cmd(["ip", "rule", "add", "from", "10.0.0.0/8", "iif", f"ipip-{machine_name}", "lookup", "100", "pref", "101"])
        self.run_cmd(["ip", "rule", "add", "oif", f"ipip-{machine_name}", "lookup", "100", "pref", "102"])
        
        self.setup_enhanced_acceleration(machine_name, f"ipip-{machine_name}", resource_plan)
    ...
    def run_cmd(self, cmd, show_output=False, check=False, quiet=False, timeout=360, shell=False):
        ...
        if self.node_type != "Moat":
            ...
            result = loop.run_until_complete(run_cmd_async(self.conn, cmd_str, check, quiet, show_output))
            return result
